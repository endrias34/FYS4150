...
class solver
{
    public:
        solver();
        solver(planet* bodies, int n, bool implicit_sun = false);
    ...
    private:
        ...
        void compute_energy(double time,int frame_write, int frame) const;
        void compute_angular_momentum(double time, int frame_write, int frame) const;
