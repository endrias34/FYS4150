...
int main(int argc, char const *argv[])
{
   ...
   * [code.exe] [scenario] [method] [max. time] [Step size] [tw_fact]
   * [scenario]    --> 2Body, 3Body, 3BodyFull, SolarSystem, Mercury_GR 
   * [method]      --> verlet, euler 
   ...
   * */
    ...
    if (scenario.compare("2Body") == 0)
    {
        ...
        //planet earth("earth", 6.0E+24, 1.0, 0.0, 0.0, 0.0, 5, 0.0);
        //planet earth("earth", 6.0E+24, 1.0, 0.0, 0.0, 0.0, 2.0*cnst::pi*sqrt(2), 0.0);
        planet earth("earth", 6.0E+24, 1.0, 0.0, 0.0, 0.0, 2.0*cnst::pi*sqrt(2)*0.99, 0.0);
        planets[0] = earth;
    }
    ...
    else if (scenario.compare("3Body") == 0)
    {
        ...
        planet jupiter("jupiter", 1000*1.9E+27, -4.633988541075995E+00, -2.854313805178032E+00, 1.155444133602380E-01,
                       3.870325272607268E-03*365, -6.074720855944709E-03*365, -6.135557504730335E-05*365);
        planets[0] = earth;
        planets[1] = jupiter;
    ...
    solver solar_system(planets, num_planets, fixed_sun);      // Create solver object
    solar_system.solve(h, t_max, t_write, method);      // Solve nbody-system using specified method
    //solar_system.compute_energy(0.0);
    delete[] planets;
